...
const baseStyles = StyleSheet.create({
    ...
    header: { marginBottom: 14, borderBottomWidth: 2, borderBottomColor: '#4f46e5', paddingBottom: 8 },
    name: { fontSize: 26, fontWeight: 'bold', marginBottom: 6, color: '#4f46e5' },
    ...
    sectionTitle: { fontSize: 14, fontWeight: 'bold', textTransform: 'uppercase', marginBottom: 6, color: '#4f46e5', borderBottomWidth: 1, borderBottomColor: '#eee', paddingBottom: 4 },
    section: { marginBottom: 8 },
    summaryText: { lineHeight: 1.3 },
    itemBlock: { marginBottom: 8 },
    itemHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 2 },
    itemTitle: { fontWeight: 'bold', color: '#000' },
    itemDate: { color: '#4f46e5', fontSize: 10 },
    itemSubtitle: { fontStyle: 'italic', marginBottom: 4 },
    bulletPoint: { flexDirection: 'row', marginBottom: 2 },
    bulletText: { flex: 1, lineHeight: 1.4 },
    bulletIcon: { width: 10, marginRight: 2, color: '#4f46e5' },
    ...
    website: { fontSize: 10, color: '#4f46e5', textDecoration: 'underline' },
});
...
export const ModernTemplate = ({ data }: { data: any }) => {
    ...
    const baseFontSize = FONT_SIZE_MAP[fontSize as keyof typeof FONT_SIZE_MAP] || 11;
    const baseLineSpacing = LINE_SPACING_MAP[lineSpacing as keyof typeof LINE_SPACING_MAP] || 1.25;
    const pageMargin = MARGIN_MAP[margin as keyof typeof MARGIN_MAP] || 30;
                    ...
                    {projects.map((item: any, index: number) => {
                        ...
                        return (
                            <View key={index} style={styles.itemBlock}>
                                ...
                                {highlightsArray && highlightsArray.length > 0 && highlightsArray.map((highlight: string, hIdx: number) => (
