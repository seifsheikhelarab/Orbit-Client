...
import { Plus, GripVertical, Trash2, ChevronDown, FolderKanban } from "lucide-react";
        ...
        >
            <div className="flex items-center gap-2 px-4 py-3">
                ...
                <button
                    ...
                    aria-label={`${useWatch({ control, name: `projects.${index}.name` }) || `Project ${index + 1}`} section, ${isOpen ? 'expanded' : 'collapsed'}`}
                        ...
                        <div className="col-span-2 space-y-1.5">
                            <Label htmlFor={`projects-${index}-description`} className="text-[10px] font-bold text-on-surface-variant uppercase tracking-tight">Description</Label>
                             <Input
                                 ...
                                 placeholder="A full-stack e-commerce platform with real-time features"
                                 className="bg-surface border-outline rounded-lg focus-visible:ring-2 focus-visible:ring-primary/20 h-10 px-3 placeholder:text-on-surface-variant"
                             />
                        </div>
...
export function Projects({ control, register }: ProjectsProps) {
            ...
            <div className="p-6">
                {fields.length === 0 ? (
                    <div className="text-center py-8 bg-surface-container-low rounded-xl border border-dashed border-outline">
                        <FolderKanban className="w-8 h-8 text-on-surface-variant/60 mx-auto mb-2" />
