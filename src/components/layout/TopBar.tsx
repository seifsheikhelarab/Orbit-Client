...
function NotificationBell() {
  ...
  return (
     ...
     >
       ...
       {count > 0 && (
         <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 flex items-center justify-center rounded-full bg-primary text-on-primary text-label-sm font-semibold leading-none animate-in zoom-in-95 duration-300 animate-click-ripple">
           {count > 99 ? '99+' : count}
         </span>
       )}
     </button>
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between bg-surface-container-high/90 backdrop-blur-md px-6 md:px-10 py-4">
      <div className="flex items-center gap-5 text-on-surface">
        ...
        <button type="button" onClick={() => navigate('/app/dashboard')} className="flex items-center gap-3 group">
          ...
          <h2 className={cn(
            "text-on-surface text-headline-lg font-bold tracking-tight",
      ...
      <div className="flex items-center gap-3">
         <NotificationBell />
         <button type="button" onClick={() => navigate('/app/settings')} className="transition-transform duration-150 hover:scale-105" aria-label="User settings">
           <UserAvatar />
         </button>
       </div>
    </header>
